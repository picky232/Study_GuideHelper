...
- [x] 6/24 — 프로젝트 초기 세팅 (Vite, 폴더 구조, 라이브러리, Vercel 연결)
- [x] 6/25 — Supabase 인증 + DB 스키마 (이메일/비밀번호 로그인, JWT 발급)
- [x] 6/26 — 홈 대시보드 UI (목업 데이터)
- [x] 6/27 — 목표 입력 UI (F1)
- [x] 6/28 — Claude API 연동 + 계획 생성 (F2)
- [x] 6/29 — 계획 재조정 + 홈 데이터 연동 (F2)
- [x] 6/30 — 캘린더 뷰 구현 (F3)
...
- [x] 7/7 — 최종 점검 완료: 시나리오 통합 테스트 (60일·2주 플랜), 환경변수·Cron·보안(RLS + Cron 시크릿) 점검, 시연 시나리오(DEMO.md) 작성
